...
//
//  //def &&(p: Prop): Prop = {
//  //    val x:(TestCases,RNG) => Result = (tc, rng) => {
//  //      val r1 = run(tc,rng)
//  //      val r2 = p.run(tc,rng)
//  //      (r1,r2) match {
//  //        case (Passed,Passed) => Passed
//  //        case (Falsified(f,s),Passed) => Falsified(f, s + tc)
//  //        case (Passed, Falsified(f,s)) => Falsified(f, s + tc)
...
//        val x = RNG.map(RNG.nonNegativeInt)(i => start + (i % (start - stopExclusive)))
//        Gen(State(x))
//      }
//
...
//        val x: State[RNG, B] = (s: RNG) => {
//          val (a, s1) = sample.run(s)
//          val state = f(a).sample
//          state.run(s1)
//        }
//        Gen(x)
//      }
//
//      //oh yeah, you don't actually need to keep rewriting flatmap
//      def flatMap2[B](f: A => Gen[B]): Gen[B] =
//        Gen(sample.flatMap(a => f(a).sample))
//
//      //  def listOfN(size: Gen[Int]): Gen[List[A]] = {
//      //    Gen((s: RNG) => {
//      //      val l = List.fill(size.sample.run(s)._1)(sample)
//      //      RNG.sequence(l)
//      //    })
//      //  }
//
//      def listOfN(size: Gen[Int]): Gen[List[A]] = {
//        size.flatMap(x => {
//          Gen(State.sequence(List.fill(x)(sample)))
//        })
//      }
//
